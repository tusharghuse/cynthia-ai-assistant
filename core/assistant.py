...
SYSTEM_PROMPT = """
You are Cynthia, an advanced, highly capable AI personal assistant and productivity coach.
...
class Assistant:
    ...
    def start(self):
        ...
        while True:
            ...
            if any(w in user_input.lower() for w in ["quit", "exit", "goodbye", "bye"]):
                farewell = "Session over. No excuses tomorrow. Get it done."
    ...
    def process_input(self, text: str):
        """Web interface entry point for a single turn of conversation."""
        log(f"User: {text}")
    ...
    def get_messages(self):
        """Fetch messages queued for the web UI."""
    ...
    def _build_greeting(self):
        ...
        if goal:
            base = f"Cynthia online. Your goal is: {goal}. "
        else:
            base = "Cynthia online. "
    ...
    def _handle_command(self, text: str) -> bool:
        ...
        # SHOW TASKS
        if "show tasks" in t or "list tasks" in t or "my tasks" in t:
        ...
        # REMINDER — "remind me at 5 pm to drink water"
        if "remind me" in t:
        ...
        # SAVE GOAL — "my goal is to build a startup"
        if "my goal is" in t:
        ...
        # RECALL GOAL
        if "what is my goal" in t or "my goal" in t:
            goal = self.memory.get("goal")
            if goal:
        ...
        # SCHEDULE DAY
        if "schedule my day" in t or "plan my day" in t:
            tasks = self.tasks.get_pending()
            if tasks:
                task_list = ", ".join([t["name"] for t in tasks])
                prompt = f"Create a strict time-blocked daily schedule for these tasks: {task_list}. Start from 8 AM. Be concise."
            else:
