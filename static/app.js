document.addEventListener("DOMContentLoaded", () => {
    ...
    async function init() {
        ...
        try {
            ...
            if (data.greeting) appendMessage("cynthia", data.greeting, { speak: true });
        } catch (error) {
            appendMessage("cynthia", "Cynthia is having trouble reaching the local server.");
        }
    ...
    chatForm.addEventListener("submit", async (event) => {
        ...
        try {
            ...
            if (!res.ok) {
                appendMessage("cynthia", data.error || "Cynthia could not process that.");
                return;
            }

            (data.responses || []).forEach((msg) => appendMessage("cynthia", msg, { speak: true }));
            pollState();
        } catch (error) {
            removeTyping(typingId);
            appendMessage("cynthia", "System error. The local API is offline.");
        }
    });
    ...
    taskList.addEventListener("click", async (event) => {
        ...
        try {
            ...
            const data = await res.json();
            appendMessage("cynthia", data.message || "Task updated.", { speak: true });
            pollState();
        } catch (error) {
            appendMessage("cynthia", "Task action failed. Try typing the command instead.");
        }
    });
    ...
    function setupSpeech() {
        ...
        micBtn.addEventListener("click", async () => {
            ...
            try {
                ...
                chatInput.placeholder = "Ask Cynthia anything or type a command...";
    ...
    async function pollState() {
        try {
            ...
            (data.messages || []).forEach((msg) => appendMessage("cynthia", msg, { speak: true }));
        } catch (error) {
        ...
        tasks.forEach((task, index) => {
            ...
            item.innerHTML = `
                ...
                <div class="task-actions">
                    <button type="button" class="mini-button" data-action="complete" data-position="${position}" title="Mark complete" aria-label="Mark task ${position} complete">
                        <svg viewBox="0 0 24 24" aria-hidden="true"><path d="m20 6-11 11-5-5"/></svg>
                    </button>
                    <button type="button" class="mini-button danger" data-action="delete" data-position="${position}" title="Delete task" aria-label="Delete task ${position}">
    ...
    function appendMessage(sender, text, options = {}) {
        ...
        if (sender === "cynthia" && typeof marked !== "undefined") {
            contentHTML = marked.parse(text);
        }
        
        div.innerHTML = `
            <span class="sender">${sender === "cynthia" ? "Cynthia" : "You"}</span>
            <div class="content">${contentHTML}</div>
        `;
        chatMessages.appendChild(div);
        scrollToBottom();

        if (sender === "cynthia" && options.speak) speak(text);
    }
